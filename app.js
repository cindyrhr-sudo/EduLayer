...
/** Canvas einer Seite komplett aus Z.annotationen neu zeichnen. */
...
function undoAusfuehren() {
  ...
  // Aktuellen Stand ins Redo sichern
  Z.redoVerlauf[seite].push(Z.annotationen[seite]?.length ?? 0);
  // Striche auf Checkpoint zurückschneiden
  const ziel = Z.undoVerlauf[seite].pop();
  if (!Z.annotationen[seite]) Z.annotationen[seite] = [];
  Z.annotationen[seite].length = ziel;  // Array in-place kürzen
  canvasNeuZeichnen(seite);
}
...
function redoAusfuehren() {
  ...
  if (!Z.undoVerlauf[seite]) Z.undoVerlauf[seite] = [];
  Z.undoVerlauf[seite].push(Z.annotationen[seite]?.length ?? 0);
...
async function pdfLaden(datei) {
  ...
  try {
    ...
    // Die echte Viewport-Größe schätzen wir anhand von Seite 1
    if (Z.seitenAnzahl > 1) {
      const vp1 = Z.viewports[1];
      for (let i = 2; i <= Z.seitenAnzahl; i++) {
        seitePlatzhalterAnlegen(i, vp1.breite, vp1.hoehe);
      }
    }
